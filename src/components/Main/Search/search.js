...
const BeforeSearch = (history, clearAll, removeOne) => {
    const renderFindText = (text) => (
        <View style={{flexDirection: 'row', margin: 10}}>
            ...
            <TouchableOpacity onPress={() => removeOne(text)}>
                <Icon name="trash-o" size={20} color='#616161'/>
            </TouchableOpacity>
        </View>
    )
    ...
    return (
        <View style={{margin: 10}}>
            ...
            {history !== undefined ? history.map(text => renderFindText(text)) : null}
...
const Search = ({ navigation }) => {
    const [history, setHistory] = useState(['aaaaaaa', 'b']);
    ...
    return (
        <View>
            ...
            {submitted ? 
                ...
                :
                BeforeSearch(history, clearAll, removeOne)
            }
        </View>
    )
}
